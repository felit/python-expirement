...
def getPyTable(data):
    ...
    length = len(data)
    while pos < length:
        index = ss(data[pos] + data[pos + 1])
        # print index,
        pos += 2
        l = ss(data[pos] + data[pos + 1])
        # print l,
        pos += 2
...
def getChinese(data):
    ...
    while pos < length:
        # 同音词数量
        same = ss(data[pos] + data[pos + 1])
        # print '[same]:',same,

        # 拼音索引表长度
        pos += 2
        py_table_len =  ss(data[pos] + data[pos + 1])
        # 拼音索引表
        pos += 2
        ...
        for i in xrange(same):
            # 中文词组长度
            c_len = ss(data[pos] + data[pos + 1])
            # 中文词组
            pos += 2
            word = byte2str(data[pos: pos + c_len])
            # 扩展数据长度
            pos += c_len
            ext_len = ss(data[pos] + data[pos + 1])
            # 词频
            pos += 2
            count =ss(data[pos] + data[pos + 1])
...
if __name__ == '__main__':
    ...
    name = 'fangchan'
    o = ['/Users/congsl/work/self/python-expirement/nltknlp/sogou/all.scel',]
